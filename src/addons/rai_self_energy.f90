subroutine rai_self_energy(iq,iqloc,istep,nw_se,corr_se_aux)
use modmain
use mod_linresp
use mod_addons_q
use mod_nrkp
...
integer ist1,ist2
complex(8) zw(2),zt1,intw_aux1,intw_aux2
...
!open file for amat
! need to modify for correct performance
if (scgwni.gt.1) then
 write(fname,'("Temp_files/rai_amat_q",I4.4)') iq
 if (istep.eq.1) then
...
! find frequency dependent part of inverse epsilon
  ...
  if (iw.gt.1) then ! start with iw = 2,3,4,...,N
  !slope
...
! change order of indices
  ...
  do isp1=1,nspinor
   ...
   do i=1,namegqblh(ikloc)
    ...
    zw(1)=dcmplx(evalsvnr(n,ik),lr_eta)   !w
    zw(2)=dcmplx(evalsvnr(n,ik)+del_e,lr_eta)
