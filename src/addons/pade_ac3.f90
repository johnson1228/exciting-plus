...
!
allocate(gn(niw,niw))
allocate(sigcoe(niw,nstsv,1,nkptnrloc))
allocate(ctmp1(niw,nstsv,1,nkptnrloc))
allocate(ctmp2(nstsv,1,nkptnrloc,lr_nw))
allocate(gf0_iw(nstsv,1,nkptnrloc,niw))
allocate(gf0_w(nstsv,1,nkptnrloc,niw))
!
sigcoe=zzero
gf0_w=zzero
gf0_iw=zzero
...
call ft_tw4(1,-1,nstsv,1,nkptnrloc,lr_nw,niw,ctmp2,gf0_iw)
 ...
 do ist1=qpnb(1),qpnb(2)
  ...
  do iw1=1,niw
    ...
    write(168,'(5G18.8)') dimag(wn(iw1))*ha2ev,&
                &dreal(gf0_iw(ist1,1,ikloc,iw1))/ha2ev,&
                &dimag(gf0_iw(ist1,1,ikloc,iw1))/ha2ev,&
...
do ikloc=1,nkptnrloc
  do ist1=1,nstsv
   do iw1=1,niw
    do iw2=iw1,niw
     if (iw1.eq.1) then
      gn(1,iw2)=gf0_iw(ist1,1,ikloc,iw2)
     else
...
! gn(i,i) are the coefficients of the fitted self-energy
    sigcoe(iw1,ist1,1,ikloc)=gn(iw1,iw1)
   enddo !iw1
  enddo !ist1
enddo !ikloc
...
do ikloc=1,nkptnrloc
  ...
  do ist1=1,nstsv
   do iw1=1,niw
    ...
    gf0_w(ist1,1,ikloc,iw1)=ac_func(niw,sigcoe(:,ist1,1,ikloc),wn,enk)
!    if (ik.eq.11.and.ist1.eq.qpnb(1)) write(*,*) "gf0_w:",gf0_w(ist1,ikloc,iw1)
     if (ik.eq.11.and.ist1.eq.qpnb(1)) write(*,*) "sigcoe:",sigcoe(iw1,ist1,1,ikloc)
   enddo
  enddo
enddo
 ...
 do ist1=qpnb(1),qpnb(2)
  ...
  do iw1=1,niw
    ...
    write(168,'(5G18.8)') dreal(enk)*ha2ev,&
                &dreal(gf0_w(ist1,1,ikloc,iw1))/ha2ev,&
                &dimag(gf0_w(ist1,1,ikloc,iw1))/ha2ev,&
                &dreal(enk1)/ha2ev,dimag(enk1)/ha2ev
  enddo
  close(168)
 enddo !ist1
