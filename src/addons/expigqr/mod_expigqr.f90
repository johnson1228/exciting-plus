...
!
!
write(fname,'("Temp_files/amegq_ip",I4.4,"_iq",I4.4)') &
      & mpi_grid_dim_pos(dim_k),iq
...
!
!
write(fname,'("Temp_files/megq_ip",I4.4,"_iq",I4.4)') &
     & mpi_grid_dim_pos(dim_k),iq
open(171,file=trim(adjustl(fname)),action='write',form='unformatted',&
& status='replace')
write(171) iq
...
&   status='old')
read(171) q0
if (q0.ne.iq) write(*,*) "q0 not equal to iq",q0, iq
read(171) ngq(iq)
read(171) amegqblh
read(171) namegqblh
read(171) bamegqblh
