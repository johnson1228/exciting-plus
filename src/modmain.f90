...
! flag for vxcval
logical rho_val
! flag for partial core density
logical pt_core
! minimal core state index of each atomic species for pt_core
integer,allocatable :: crst_in(:)
! muffin-tin charge density
real(8), allocatable :: rhomt(:,:,:)
! interstitial real-space charge density
real(8), allocatable :: rhoir(:)
! muffin-tin valence charge density
real(8), allocatable :: rhomt_val(:,:,:)
! interstitial valence real-space charge density
real(8), allocatable :: rhoir_val(:)
...
! muffin-tin exchange-correlation potential from valence density
real(8), allocatable :: vxcmt_val(:,:,:)
! interstitial real-space exchange-correlation potential from valence density
real(8), allocatable :: vxcir_val(:)
...
! total core charge
real(8) chgcr
! partial core charge for pt_core
real(8) ptchgcr
! core leakage charge
real(8) chgcrlk
...
real(8) chgcalc
! calculated total valence charge
real(8) chgcalc_val
! interstitial region charge
real(8) chgir
! intestitial region valence charge
real(8) chgir_val
! muffin-tin charges
real(8), allocatable :: chgmt(:)
! muffin-tin valence charges
real(8), allocatable :: chgmt_val(:)
! total muffin-tin charge
real(8) chgmttot
! total muffin-tin valence charge
real(8) chgmttot_val
...
real(8) fermidos
! estimated indirect and direct band gap
real(8) bandgap(2)
...
! radial partial core density
real(8), allocatable :: ptrhocr(:,:,:)
...
! vc-relax
integer latvopt
data latvopt /0/ !no vc-relax by default
...
! maximum steps in structual optimization (fixed cell)
integer :: maxatmstp
data maxatmstp /200/
! maximum steps in structual optimization (vc-relax)
integer :: maxlatvstp
data maxlatvstp /100/
! minimum allowed distance between muffin-tin surfaces
real(8) rmtdelta
data rmtdelta /0.05d0/
