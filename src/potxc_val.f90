...
! compute the density in spherical coordinates
    call dgemm('N','N',lmmaxvr,nr,lmmaxvr,1.d0,rbshtvr,lmmaxvr,rhomt_val(:,:,ias), &
...
!         lmmaxvr,0.d0,bxcmt_val(:,:,ias,idm),lmmaxvr)
!      end do
    else
!--------------------------!
!     spin-unpolarised     !
...
! non-collinear
    do ir=1,ngrtot
      ...
      rhoup(ir)=0.5d0*(rhoir_val(ir)+t1)
      rhodn(ir)=0.5d0*(rhoir_val(ir)-t1)
    end do
  else
! collinear
    do ir=1,ngrtot
      rhoup(ir)=0.5d0*(rhoir_val(ir)+magir(ir,1))
      rhodn(ir)=0.5d0*(rhoir_val(ir)-magir(ir,1))
...
! non-collinear: spin rotate the local exchange potential
    do ir=1,ngrtot
      t1=vxup(ir)+vcup(ir)
      t2=vxdn(ir)+vcdn(ir)
      vxcir_val(ir)=0.5d0*(t1+t2)
! determine the exchange-correlation magnetic field
!      t3=0.5d0*(t1-t2)
!      t4=rhoup(ir)-rhodn(ir)
!      if (abs(t4).gt.1.d-8) t4=t3/t4
!      bxcir_val(ir,:)=magir(ir,:)*t4
    end do
  else
! collinear
    do ir=1,ngrtot
...
!--------------------------!
  if (xcgrad.le.0) then
    call xcifc(xctype,n=ngrtot,rho=rhoir_val,ex=exir,ec=ecir,vx=vx,vc=vc)
  else if (xcgrad.eq.1) then
    call ggair_1(grho,g2rho,g3rho)
    call xcifc(xctype,n=ngrtot,rho=rhoir_val,grho=grho,g2rho=g2rho,g3rho=g3rho, &
     ex=exir,ec=ecir,vx=vx,vc=vc)
  else if (xcgrad.eq.2) then
    call ggair_2a(g2rho,gvrho,grho2)
    call xcifc(xctype,n=ngrtot,rho=rhoir_val,grho2=grho2,ex=exir,ec=ecir,vx=vx, &
     vc=vc,dxdg2=dxdg2,dcdg2=dcdg2)
    call ggair_2b(g2rho,gvrho,vx,vc,dxdg2,dcdg2)
  end if
  vxcir_val(1:ngrtot)=vx(1:ngrtot)+vc(1:ngrtot)
end if
...
call symrf(1,vxcmt_val,vxcir_val)
...
!  call symrvf(1,bxcmt_val,bxcir_val)
end if
