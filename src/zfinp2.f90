...
! !ROUTINE: zfinp
! !INTERFACE:
complex(8) function zfinp2(tsh,zfmt1,zfmt2,zfir1,zfir2,nr,rrmt)
! !USES:
use modmain
...
real(8), intent(in) :: rrmt(nrmtmax,nspecies)
! local variables
...
do is=1,nspecies
  do ia=1,natoms(is)
    ias=idxas(ia,is)
    zsum=zsum+zfmtinp(tsh,lmaxvr,nr(is),rrmt(:,is),lmmaxvr,zfmt1(:,:,ias), &
     zfmt2(:,:,ias))
  end do
end do
zfinp2=zsum
