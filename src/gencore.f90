...
! append the effective potential from the atomic calculation for r > R^MT
        ...
        if (pt_core) ptrhocr(:,ias,ispn)=0.d0
        do ist=1,spnst(is)
...
! add to the core density
            ...
            ! if only partial core density is required
            if (pt_core) then
             if (ist.ge.crst_in(is)) then
              do ir=1,spnr(is)
                ptrhocr(ir,ias,ispn)=ptrhocr(ir,ias,ispn) &
                 +t1*(rwfcr(ir,1,ist,ias)**2+rwfcr(ir,2,ist,ias)**2)
              end do
             endif
            endif
        ...
        if (pt_core) then ! partial core
          do ir=1,spnr(is)
            ptrhocr(ir,ias,ispn)=ptrhocr(ir,ias,ispn)/(fourpi*spr(ir,is)**2)
          end do
        endif
